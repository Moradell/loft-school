...
function returnBadArguments(fn, ...args) {
  ...
  for (let i = 1; i < arguments.length; i++) {
    try { fn(arguments[i]) }
    catch(e) { array.push(arguments[i]) }
  }
...
function calculator(number = 0) {
  ...
  let obj = {
    sum: function (...args) {
      return args.reduce((previousValue, current) => previousValue + current, number);
    },
    ...
    div: function (...args) {
      ...
      let dealDiv = (previousValue, current) => {
      ...
      return args.reduce(dealDiv, number);
    },
