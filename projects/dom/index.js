...
function findAllPSiblings(where) {
  let elementChild = where.children;
  let array = [];

  for (let i = 0; i < elementChild.length - 1; i++) {
    if (elementChild[i].nextElementSibling.tagName == "P") {
      array.push(elementChild[i]);
    }
  }
  return array;
}
...
function deleteTextNodes(where) {
  let elementChild = where.childNodes;

  for (const child of elementChild) {
    if (child.nodeType === 3) {
...
function deleteTextNodesRecursive(where) {
  ...
  for (let i = 0; i < elementChild.length; i++) {
    let nthChild = elementChild[i];

    if (nthChild.nodeType === 3) {
      where.removeChild(nthChild);
      i--;
    } else if (nthChild.nodeType === 1) {
      deleteTextNodesRecursive(nthChild);
    }
  }
}
